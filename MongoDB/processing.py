...
def process_file(filename, fields):
    ...
    with open(filename, 'r') as f:
        ...
        for line in reader:
            #data_point = {}
            data_point = collections.defaultdict(dict)
            for f in fields:
                #data_point['classification'] = {}
                if '_label' in f:
                    try:
                        if line[f].startswith('{'):
                            data_point['classification'][fields[f]] = parse_array(line[f])
                        elif line[f] == 'NULL' or line[f] == '':
                            data_point['classification'][fields[f]] = None
                        elif int(line[f]):
                            data_point['classification'][fields[f]] = int(line[f])
                    except ValueError:
                        try:
                            if float(line[f]):
                                data_point['classification'][fields [f]] = float(line[f])
                        except ValueError:
                            data_point['classification'][fields[f]] = line[f]
                elif f == 'synonym':
                    if line[f] == 'NULL' or line[f] == '':
                        data_point[fields[f]] = None
                    else:
                        data_point[fields[f]] = parse_array(line[f])
                elif f == 'rdf-schema#label':
                    data_point[fields[f]] = re.sub(' \([^)]*\)', '', line[f])
                elif f == 'name' and (line[f] == 'NULL' or line[f] == ''):
                    data_point[fields[f]] = data_point['label']
                    ...
                    try:
                        if line[f].startswith('{'):
                            ...
                            data_point[fields[f]] = int(line[f])
                        
                    except ValueError:
                        try:
                            if float(line[f]):
                                data_point[fields [f]] = float(line[f])
                        except ValueError:
                            data_point[fields[f]] = line[f]
